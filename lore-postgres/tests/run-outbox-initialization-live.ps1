...
$containerName = "wp116-outbox-initialization-live-$runId"
$ownershipLabelName = 'com.tideshift.lore.outbox-initialization-live'
...
$target = 'domain_outbox_initialization'
...
$results += [pscustomobject]@{
    ...
    Test = 'migration_file_and_boot_time_ensure_schema_produce_identical_domain_catalogs'
    Status = 'NOT RUN'
    Passed = 0
    Failed = 0
    Ran = 0
...
function Get-TestCatalog {
    ...
    try {
        ...
        $listArgs = @('test', '-j', '4', '-p', $package, '--test', $target, '--', '--ignored', '--list')
...
function Assert-NoCollidingContainer {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'failed to inspect existing outbox-initialization live containers'
    }
    $collisions = @($raw | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
    if ($collisions.Count -ne 0) {
        $message = "another outbox-initialization live container exists; refusing to overlap:`n" +
            ($collisions -join "`n")
        throw $message
    }
}
...
try {
    ...
    Invoke-Checked docker @(
        ...
        '--label', "com.tideshift.lore.outbox-initialization-live.pid=$PID",
        '--label', "com.tideshift.lore.outbox-initialization-live.started=$([DateTime]::UtcNow.ToString('yyyy-MM-ddTHH:mm:ssZ'))",
    ...
    try {
        ...
        foreach ($result in $results) {
            ...
            try {
                $ErrorActionPreference = 'Continue'
                $cargoArgs = @(
                    'test', '-j', '4', '-p', $package, '--test', $result.Target,
    ...
    if ($containerCreationAttempted -and ($runPassed -or -not $KeepOnFailure)) {
        ...
        $actualPid = if ($null -ne $labels) {
            $labels.PSObject.Properties['com.tideshift.lore.outbox-initialization-live.pid'].Value
        }
