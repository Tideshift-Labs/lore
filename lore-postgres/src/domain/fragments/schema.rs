...
mod tests {
    ...
    fn seeded_schema_version(ddl: &str, source: &str) -> i64 {
        let seed = ddl
            .split_once("INSERT INTO lore_fragment_schema_state (")
            .unwrap_or_else(|| panic!("{source} must contain the fragment schema-state seed"))
            .1
            .split(';')
            .next()
            .unwrap();
    ...
    #[test]
    fn schema_version_probe_ignores_other_tables_before_and_after_the_fragment_seed() {
        let ddl = "INSERT INTO other_schema (schema_version) SELECT\n99 AS schema_version;\n\
            INSERT INTO lore_fragment_schema_state (schema_version) SELECT\n17 AS schema_version;\n\
            INSERT INTO later_schema (schema_version) SELECT\n88 AS schema_version;";
        assert_eq!(seeded_schema_version(ddl, "fixture"), 17);
        let missing = ddl.replace("17 AS schema_version", "17 AS unrelated_column");
        assert!(
            std::panic::catch_unwind(|| seeded_schema_version(&missing, "missing alias")).is_err()
        );
    }
