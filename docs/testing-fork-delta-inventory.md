...
- **A-32 `live_only`/`durable` event plane and its offline switch [SERVER]**: `[notification]
  event_plane`, `DomainContext::outbox_cell_id()` (the predicate all 13 producer sites plus
  `lock_service.rs` and the fragment coordinator read instead of `cell_id()` — confirmed by a
  source-wide grep, zero remaining `.cell_id()` reads for an event decision), the cell marker/
  evidence tables in `migrations/0005_outbox_event_plane.sql`, and `loreserver outbox set-plane`
  (`lore-postgres/src/domain/outbox/event_plane.rs`, `lore-server/src/event_relay/plane.rs`). The
  resolution matrix and `check_marker` are pure and unit-tested in both modules'
  `#[cfg(test)]`. The offline switch's transactional mechanics (pending-row refusal, verbatim
  `broker_accepted`/`consumer_safe` move to `lore_outbox_retired_events` with audit,
  `AlreadyCurrent` rerun, durable-with-rows refusal, the `numbackends` and `ACCESS EXCLUSIVE
  NOWAIT` contention gates) are live-proven in `lore-postgres/tests/domain_outbox_event_plane.rs`
  (7/7 PASS, `run-event-plane-live.ps1`). Gates: `cargo test -p lore-postgres --lib
  domain::outbox::event_plane::tests` `cargo test -p lore-server --lib event_relay::plane::tests`;
  live: `pwsh -File lore-postgres/tests/run-event-plane-live.ps1`,
  `PASS=7 FAIL=0 NOT RUN=0 EXPECTED=7` at last run. Durable-mode regression reconfirmed unaffected:
  `run-outbox-initialization-live.ps1` 10/10, `run-outbox-producers-live.ps1` 29/29,
  `run-domain-enforcement-live.ps1` 11/11. **Known gap**: no live test drives a real governed
  mutation through `DomainContext`/`Governed*` end to end under `live_only` to prove zero outbox
  rows append across all producer kinds (it would need `p12_live.rs`'s receipt/digest/admission
  scaffolding); covered instead by the unit predicate plus the call-site sweep above. See
