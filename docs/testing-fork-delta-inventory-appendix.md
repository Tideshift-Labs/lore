# Fork-delta inventory Appendix

This file contains detailed historical context and architectural invariants extracted from the main inventory.

## CR-033 cell dispatch authority: continuity family and separate-process service shell removed   [SERVER]

(the cell's own PostgreSQL is the one authority; the CR-033 record, not this guide,
...
## CR-033 charge-admission deadline horizon guard [SERVER]
...
## Exact-selection commit transaction [CLIENT]

`lore-revision/tests/exact_selection_transaction.rs`
...
## State-block local retention on remote fetch [CLIENT]

`load_fragment`'s remote-fetch cache
...
## Tree-block local retention [CLIENT], companion to the entry above

the `lore-storage` fix
...
## `revision info --delta`'s delta-read-failure surfacing [CLIENT]
...
## CR-029 D2 amendment: platform-ordered completion sequencing [SERVER]
...
## CR-030 lock fencing [SERVER, WP-117]

`tests/run-lock-fencing-live.ps1` is the only evidence,
...
## CR-029 WP-116 Phase 4, gRPC metadata carriage, status mapping, and the admission gate   [SERVER]

offline, no Postgres. `domain_operation_metadata.rs`'s `extract`/`require` (R-BLOCK-2's
...
## CR-035 write-behind backpressure is not unreadiness [SERVER,   `lore-server/src/fragment_write_behind.rs`]

`classify_pass`/`record_pass_outcome`/
...
`staged_drain_candidates` (bounded plan query, `lore-postgres`) and
