...
//!
//! `lore-server` and `lorehub` are sibling checkouts under one `lorehub-all` container in our own
//! workspace, where the fixtures are always present. A **standalone** checkout of just this repo
//! (an upstream contributor's clone, or a fork clone without `lorehub` beside it) has no such
//! sibling at all -- that is a different fact from a fixture being merely missing or unreadable
//! *within* an existing sibling, and the two deserve different outcomes:
//!
...
//! plain executed `#[test]` (see `remote_notification_conformance.rs`, the one consuming binary
//! that carries this module's own tests) can drive every branch -- including the standalone one --
