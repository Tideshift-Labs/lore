    ...
    format!(
        "SET SESSION AUTHORIZATION object_dispatch_retention_maintenance;\
         ...
         SELECT object_store_retention.object_store_dispatch_publish_budget_configuration_v1(\
           ...
           '[{{\"dimensionId\":\"all\",\"effectiveBound\":67,\"measuredLoad\":1,\"targetDemand\":1,\"failureReserve\":1,\"preCacheHeadroom\":64,\"finalBudget\":64}}]'::jsonb,\
           '{caps}'::jsonb);\
...
fn cap_json() -> String {
    let entries = (1..=7)
        .map(|class| {
            let capacity = if class == 1 { 65 } else if class == 7 { 1 } else { 64 };
            format!(
