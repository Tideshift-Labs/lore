...
# Relational CD8 fixtures and real grants only; no write-behind lifecycle certification.
[CmdletBinding()]
param([switch]$KeepOnFailure)
$ErrorActionPreference = 'Stop'
$runId = [Guid]::NewGuid().ToString('N')
$container = "wp114-cell-retention-live-$runId"
$labelKey = 'com.tideshift.lore.cell-retention-live'
...
$started = $false
$passed = $false
$envNames = @('LORE_TEST_CELL_RETENTION_PG_URL', 'LORE_TEST_CELL_RETENTION_CA_PATH')
$prior = @{}
...
try {
 ...
 try {
  $catalogArgs = @('test', '-p', 'lore-object-dispatch', '--test', 'cell_retention_live', '-j', '4', '--', '--ignored', '--list')
 ...
 foreach ($test in $tests) {
  ...
  Push-Location $loreRoot
  try {
   $cargoArgs = @('test', '-p', 'lore-object-dispatch', '--test', 'cell_retention_live', '-j', '4', '--', '--ignored', '--exact', $test, '--test-threads=1', '--nocapture')
 ...
 $passed = $true
 Write-Host "$($tests.Count) passed; 0 failed; 0 skipped. Write-behind lifecycle is not certified."
} finally {
