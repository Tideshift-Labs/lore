...
-- WP-121 Phase 2 source-dark PUT-reservation provisioning and readback.

BEGIN;
SET LOCAL ROLE object_dispatch_retention_owner;

ALTER TABLE object_store_retention.object_dispatch_retention_schema_state
  ADD COLUMN put_reservation_schema_revision text,
  ADD COLUMN put_reservation_migration_blake3 object_store_retention.blake3_256,
  ADD COLUMN put_reservation_install_revision object_store_retention.uint64,
  ADD COLUMN put_reservation_installed_at_unix_ms bigint,
  ADD CONSTRAINT object_dispatch_retention_schema_state_put_reservation_ck CHECK (
    (
      ...
      put_reservation_schema_revision = 'object-store-dispatch-put-reservation-schema-v1' AND
      put_reservation_install_revision > 0 AND
...
CREATE FUNCTION object_store_retention.assert_dispatch_put_reservation_provisioning_api_revision_v1(
  api_revision text
)
RETURNS void
LANGUAGE plpgsql
...
AS $$
BEGIN
  IF api_revision IS DISTINCT FROM 'object-store-dispatch-put-reservation-provisioning-v1' THEN
    RAISE EXCEPTION 'UNSUPPORTED_DISPATCH_PUT_RESERVATION_API_REVISION'
      USING ERRCODE = '22023';
  END IF;
END
$$;
...
BEGIN
  ...
  SELECT pg_catalog.concat_ws(
    ...
    'types=' || COALESCE((
      SELECT pg_catalog.jsonb_agg(pg_catalog.jsonb_build_array(
        ...
        type_state.typispreferred,
        type_state.typisdefined,
        type_state.typdelim,
        pg_catalog.pg_get_userbyid(type_state.typowner),
        type_state.typnotnull,
        CASE WHEN type_state.typbasetype = 0 THEN NULL
             ELSE pg_catalog.format_type(type_state.typbasetype, type_state.typtypmod)
        END,
        CASE WHEN type_state.typelem = 0 THEN NULL
             ELSE pg_catalog.format_type(type_state.typelem, NULL)
        END,
        CASE WHEN type_state.typrelid = 0 THEN NULL
             ELSE type_relation.relname
        END,
        CASE WHEN type_state.typcollation = 0 THEN NULL
             ELSE pg_catalog.format('%I.%I', collation_namespace.nspname, collation_state.collname)
        END,
        ...
        JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = type_state.typnamespace
        LEFT JOIN pg_catalog.pg_class AS type_relation ON type_relation.oid = type_state.typrelid
        LEFT JOIN pg_catalog.pg_collation AS collation_state
          ON collation_state.oid = type_state.typcollation
        LEFT JOIN pg_catalog.pg_namespace AS collation_namespace
          ON collation_namespace.oid = collation_state.collnamespace
       WHERE namespace.nspname = 'object_store_retention'
    ), '[]'),
    'composite_attributes=' || COALESCE((
      SELECT pg_catalog.jsonb_agg(pg_catalog.jsonb_build_array(
        ...
        CASE WHEN attribute.attcollation = 0 THEN NULL
             ELSE pg_catalog.format('%I.%I', collation_namespace.nspname, collation_state.collname)
        END
      ) ORDER BY type_state.typname, attribute.attnum)::text
        FROM pg_catalog.pg_type AS type_state
        JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = type_state.typnamespace
        JOIN pg_catalog.pg_attribute AS attribute ON attribute.attrelid = type_state.typrelid
        LEFT JOIN pg_catalog.pg_collation AS collation_state
          ON collation_state.oid = attribute.attcollation
        LEFT JOIN pg_catalog.pg_namespace AS collation_namespace
          ON collation_namespace.oid = collation_state.collnamespace
    ...
    'domain_constraints=' || COALESCE((
      ...
      ) ORDER BY type_state.typname, constraint_state.conname)::text
        FROM pg_catalog.pg_constraint AS constraint_state
        JOIN pg_catalog.pg_type AS type_state ON type_state.oid = constraint_state.contypid
        JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = type_state.typnamespace
       WHERE namespace.nspname = 'object_store_retention'
    ), '[]'),
    'type_acls=' || COALESCE((
      ...
      ) ORDER BY type_state.typname, privilege.grantee, privilege.privilege_type)::text
        FROM pg_catalog.pg_type AS type_state
        JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid = type_state.typnamespace
    ...
    'functions=' || COALESCE((
      SELECT pg_catalog.jsonb_agg(pg_catalog.jsonb_build_array(
        ...
        CASE
          WHEN procedure.proname = 'assert_dispatch_put_reservation_catalog_v1' THEN
  ...
  IF catalog_manifest_sha256 IS DISTINCT FROM
     pg_catalog.decode('837aa8d2654cea2204e88fcc56d4cd291199c73829aa77c0e55b69544864e32c', 'hex') THEN
    RAISE EXCEPTION 'DISPATCH_AUTHORITY_CATALOG_MISMATCH'
      USING ERRCODE = '55000',
            DETAIL = pg_catalog.encode(catalog_manifest_sha256, 'hex');
  END IF;
...
RETURNS object_store_retention.dispatch_put_reservation_provisioning_state_v1
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = pg_catalog
...
BEGIN
  PERFORM object_store_retention.assert_dispatch_put_reservation_catalog_v1();
  ...
  IF schema_state.schema_revision IS DISTINCT FROM
     ...
     OR schema_state.put_reservation_installed_at_unix_ms IS NULL THEN
    RAISE EXCEPTION 'DISPATCH_PUT_RESERVATION_UNAVAILABLE' USING ERRCODE = '55000';
  END IF;
  RETURN ROW(
    result_code,
  ...
  )::object_store_retention.dispatch_put_reservation_provisioning_state_v1;
...
BEGIN
  ...
  PERFORM object_store_retention.assert_dispatch_put_reservation_provisioning_api_revision_v1(
    api_revision
  );
  ...
  IF expected_schema_revision IS DISTINCT FROM
    ...
    RAISE EXCEPTION 'DISPATCH_PUT_RESERVATION_INSTALL_CONTRACT_MISMATCH'
      USING ERRCODE = '22023';
  END IF;
  ...
  IF stored.schema_revision IS DISTINCT FROM
       'object-store-retention-authority-schema-v1'
     OR stored.migration_blake3 IS DISTINCT FROM
        pg_catalog.decode('f86d1a574cab9346ef39843fed6ffb849cafe5967881a45d0c6d89028780f6dd', 'hex')
     OR stored.install_revision IS NULL OR stored.install_revision = 0
     OR stored.local_authority_schema_revision IS DISTINCT FROM
        'object-store-dispatch-authority-schema-v1'
     OR stored.local_authority_migration_blake3 IS DISTINCT FROM
        pg_catalog.decode('d762b841bd31a37908a6ff95c2292d5abfca234fa9b7d3c0c639ec63dcf3a7ff', 'hex')
     OR stored.local_authority_install_revision IS NULL
     OR stored.local_authority_install_revision = 0 THEN
    RAISE EXCEPTION 'DISPATCH_PUT_RESERVATION_UNAVAILABLE' USING ERRCODE = '55000';
  END IF;

  IF stored.put_reservation_schema_revision IS NOT NULL THEN
    IF stored.put_reservation_schema_revision IS DISTINCT FROM expected_schema_revision
       OR stored.put_reservation_migration_blake3 IS DISTINCT FROM expected_migration_blake3
       OR stored.put_reservation_install_revision IS DISTINCT FROM expected_install_revision
      ...
      RAISE EXCEPTION 'DISPATCH_PUT_RESERVATION_INSTALL_REPLAY_CONFLICT'
        USING ERRCODE = '40001';
    END IF;
    RETURN object_store_retention.project_dispatch_put_reservation_state_v1('REPLAY');
  END IF;

  IF pg_catalog.num_nonnulls(
       stored.put_reservation_migration_blake3,
       stored.put_reservation_install_revision,
       stored.put_reservation_installed_at_unix_ms
     ) <> 0
     OR EXISTS (SELECT 1 FROM object_store_retention.object_dispatch_requests)
     OR EXISTS (SELECT 1 FROM object_store_retention.object_dispatch_attempts)
     OR EXISTS (SELECT 1 FROM object_store_retention.object_dispatch_spool_objects)
     ...
     OR EXISTS (SELECT 1 FROM object_store_retention.object_dispatch_payload_purges)
     OR EXISTS (SELECT 1 FROM object_store_retention.object_dispatch_fetch_leases) THEN
    RAISE EXCEPTION 'DISPATCH_PUT_RESERVATION_INSTALL_DIRTY_STATE' USING ERRCODE = '55000';
  END IF;

  installed_at := object_store_retention.clock_unix_ms_v1();
  UPDATE object_store_retention.object_dispatch_retention_schema_state
     SET put_reservation_schema_revision = expected_schema_revision,
         put_reservation_migration_blake3 = expected_migration_blake3,
         put_reservation_install_revision = expected_install_revision,
         put_reservation_installed_at_unix_ms = installed_at
   WHERE singleton;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'DISPATCH_PUT_RESERVATION_UNAVAILABLE' USING ERRCODE = '55000';
  END IF;
  RETURN object_store_retention.project_dispatch_put_reservation_state_v1('CREATED');
EXCEPTION WHEN no_data_found OR too_many_rows THEN
  RAISE EXCEPTION 'DISPATCH_PUT_RESERVATION_UNAVAILABLE' USING ERRCODE = '55000';
END
$$;
...
BEGIN
  ...
  RETURN object_store_retention.project_dispatch_put_reservation_state_v1('READ');
END
$$;
...
GRANT EXECUTE ON FUNCTION
  object_store_retention.object_store_dispatch_put_reservation_install_v1(
    text, text, bytea, object_store_retention.uint64
  )
TO object_dispatch_retention_migrator;
GRANT EXECUTE ON FUNCTION
  object_store_retention.object_store_dispatch_put_reservation_read_state_v1(text)
